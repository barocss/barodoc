...
The `Guides` group shows **sidebar hierarchy**: `{ "label": "Setup", "pages": [...] }` renders as an expandable row. See [Configuration](/docs/guides/configuration#navigation) for details.
